...
char* parseRequest(char* sInput, int socket) {
    //parse the input string
	char* filepath = (sInput, "\0");
    //char* filepath = "./doc/index.html\0"; 
    //then open the file.
    //For GET requests
    FILE *file = fopen(&filepath, "r");
    while (!feof(filepath)) {
                    int numread = fread(send_buffer, sizeof(unsigned char), 1000, filepath);
                    int numsent = send(socket, *send_buffer, numread, 0);
                 }
};

void sendFile(FILE *file) {
    while (!feof(&file)) {
        //int numread = fread(send_buffer, sizeof(unsigned char), 1000, &file);
    }
}
